...
class SignUpController extends GetxController {
  ...
  List<String> profileCreators = [
    ...
    translations.parents.tr,
    translations.sister.tr,
    translations.brother.tr,
    translations.guardian.tr,
  ];
  List<String> heights = [
    "4 ft",
    "4 ft 1 inches",
        ...
        case 1:
          ...
          result = await _membershipRepository
            ...
            basicDetails.value.toJson(),
              ...
              .then((value) async {
            religiousDetails.value =
                ReligiousDetails(religion: "NA", caste: "NA", manglik: "NA");
            // religion: "Buddhist", caste: "Any", manglik: "NO");
      ...
      if (result) {
        Get.showSnackbar(GetSnackBar(
          ...
          message: isAuthenticated.value
              ? translations.successUpdating.tr
              : translations.successSubmiting.tr,
        ));

        if (selectedTile.value == 5) {
          fetchUploadData();
        ...
        Get.showSnackbar(GetSnackBar(
          ...
          duration: const Duration(seconds: 2),
          message: translations.errorUpdating.tr,
        ));
      }
    }
  }
  ...
  void validateAndContinue2(context) async {
    ...
    if (key.currentState!.validate()) {
      ...
      if (result) {
        ...
        if (selectedPreferenceTile.value == 3) {
          if (!isAuthenticated.value) {
            Get.showSnackbar(
              GetSnackBar(
                duration: const Duration(seconds: 3),
                message: translations.settingUpAcc.tr,
              ),
            );
            Future.delayed(const Duration(seconds: 1), () {
              Get.offAllNamed(
  ...
  void getSearchMatches(bool reinit) async {
    ...
    searchProfile.value.religion = "Buddhist";
    await _settingsRepository
